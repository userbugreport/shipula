...
You may also create static sites -- JAMStack, using tools like [Docusaurus](https://docusaurus.io/), and you need the resulting app hosted on a world class cloud -- [Amazon Web Services](http://aws.amazon.com).

Sounds like fun -- then you are going to find out you need to know a lot more things than just [node](https://nodejs.org), things like:
...
Or using a relatively expensive add-on service that can make hosting easier.
Shipula lets you use a _real_ cloud provider -- **AWS** but as easily as one command line.
...
Yep -- that's it.
One comand line `shipula deploy` and you are live.

If you want to use your own domain name -- set up [HTTPS](./cookbook/https).
