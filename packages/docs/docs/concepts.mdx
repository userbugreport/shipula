...
Shipula will spread traffic across the multiple copies of you app. You can `scale` at any time, just by telling shipula to add CPU, memory, or more copies.

### HTTPS

Setting up DNS and HTTPS can be -- hard. Shipula makes this as easy as getting a domain name from a provider and typing two commands. [Learn about it](./reference/https).
...
Shipula deals with this by setting up a shared file system that you can always reach at `/cluster_shared`. Store files, have fun.
