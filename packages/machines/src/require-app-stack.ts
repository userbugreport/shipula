...
export default Machine<Context, Schema, Events>(
  {
    initial: "checkingStyle",
    states: {
      checkingStyle: {
        always: [
          { target: "checkingForDocker", cond: "isServer" },
          { target: "maybeBuilding", cond: "isStatic" },
        ],
      },
      checkingForDocker: {
        invoke: {
  ...
  },
  {
    guards: {
      isStatic: (context) => {
        return context.deployStyle === "@shipula/static";
      },
      isServer: (context) => {
        return context.deployStyle === "@shipula/server";
      },
    },
  }
);
